...
//	window.parent.SetAutoSize(true);
	...
	btn = $('imgrect') ;
	btn.alt = btn.title = FCKLang.imgmaprect ;
	btn = $('imgcircle') ;
	btn.alt = btn.title = FCKLang.imgmapCircle ;
	btn = $('imgpoly') ;
	btn.alt = btn.title = FCKLang.imgmappoly ;
...
function getAreaHtml(area)
{
	...
	var html = '<area onmouseover=\"cvi_tip._show(event);\" onmouseout=\"cvi_tip._hide(event);\" onmousemove=\"cvi_tip._move(event);\" shape="' + area.shape + '"' +
							' coords="' + area.lastInput + '"' ;

	if (area.aalt && area.aalt!='') html+= ' alt="' + area.aalt + '"' ;
	if (area.atitle && area.atitle!='') html+= ' tooltip="' + area.atitle + '"' ;
